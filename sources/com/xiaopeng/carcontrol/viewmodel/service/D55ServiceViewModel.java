...
public class D55ServiceViewModel extends ServiceViewModel {
    ...
    protected void syncLockData(CarControlSyncDataEvent event) {
        ...
        if (!event.isUnlockResponse()) {
            LogUtils.d(TAG, "Unlock response, save currentValue: " + unlockResponseSp);
            this.mDataSync.setUnlockResponse(unlockResponseSp);
        } else {
            int unlockResponse = this.mDataSync.getUnlockResponse();
            LogUtils.d(TAG, "Unlock response, resume saved value: " + unlockResponse + ", currentValue: " + unlockResponseSp);
