...
public class SpaceCapsuleActivity extends XActivity implements ISpaceCapsuleInterface {
    ...
    private XDialog mHandExitDialog;
    ...
    private final boolean mIsAutoSeat = BaseFeatureOption.getInstance().isSupportAutoSeat();
    ...
    public void onResume() {
        ...
        if (this.mLayoutFlat) {
            ...
            if ((findFragmentById2 instanceof SpaceCapsulePrepareFragment) || (findFragmentById2 instanceof SleepCapsuleChooseFragment)) {
                this.mLayoutFlat = false;
                replaceFragment(SpaceCapsuleFragment.newInstance(this.mSubModeIndex, this.mSleepBed));
            }
    ...
    public void onSpaceCapsuleReady() {
        ...
        if (isSpaceSleepMode()) {
            ...
            boolean isSupportSpaceCapsuleGuideSetting = BaseFeatureOption.getInstance().isSupportSpaceCapsuleGuideSetting();
            boolean z = false;
            boolean z2 = isSupportSpaceCapsuleGuideSetting && !SharedPreferenceUtil.isSpaceCapsuleUGEnterShowCheckBoxSelected();
            if (!isSupportSpaceCapsuleGuideSetting && !SharedPreferenceUtil.isEnterSpaceCapsuleFromButton()) {
                z = true;
            }
            if (z2 || z) {
    ...
    public void onSpaceCapsuleRecover() {
        if (isSpaceSleepMode()) {
            ...
            if (SharedPreferenceUtil.isSpaceCapsuleUGExitShowCheckBoxSelected()) {
                showExitDialog(false);
                return;
            } else if (!this.mIsAutoSeat && !SharedPreferenceUtil.isFirstExitSpaceCapsuleMode()) {
                showHandSeatExitDialog();
                return;
            } else {
    ...
    public void onSpaceCapsuleEnd() {
        this.mSelfExit = true;
        if (SharedPreferenceUtil.isFirstExitSpaceCapsuleMode()) {
            SharedPreferenceUtil.setFirstExitSpaceCapsuleMode(false);
        }
        if (isFinishing()) {
            return;
        }
    ...
    private void exitSpaceCapsule(boolean isRestoreChair) {
        ...
        if (isRestoreChair) {
            ...
            this.mViewModel.restoreSpaceCapsuleSettings();
            int i = this.mIsAutoSeat ? R.string.space_capsule_sleep_exit_active : R.string.space_capsule_sleep_exit_active_recover;
            if (!isSpaceSleepMode()) {
                i = R.string.space_capsule_cinema_exit_active;
            }
            NotificationHelper.getInstance().showToast(i);
            SpeechHelper.getInstance().speak(ResUtils.getString(i));
            onSpaceCapsuleEnd();
        }
    ...
    private void showHandSeatExitDialog() {
        this.mSleepLayoutRecover = false;
        if (this.mHandExitDialog == null) {
            XDialog xDialog = new XDialog(this);
            this.mHandExitDialog = xDialog;
            xDialog.setTitle(R.string.space_capsule_sleep_exit_popup_title);
            this.mHandExitDialog.setMessage(R.string.space_capsule_exit_popup_msg_recover);
            this.mHandExitDialog.setPositiveButton(R.string.btn_confirm);
            this.mHandExitDialog.setNegativeButton(R.string.btn_cancel);
            this.mHandExitDialog.setPositiveButtonListener(new XDialogInterface.OnClickListener() { // from class: com.xiaopeng.carcontrol.view.-$$Lambda$SpaceCapsuleActivity$ZPT0NsSwSPcy_5OR2Zc6-Q5vaJ4
                @Override // com.xiaopeng.xui.app.XDialogInterface.OnClickListener
                public final void onClick(XDialog xDialog2, int i) {
                    SpaceCapsuleActivity.this.lambda$showHandSeatExitDialog$6$SpaceCapsuleActivity(xDialog2, i);
                }
            });
            this.mHandExitDialog.setNegativeButtonListener(new XDialogInterface.OnClickListener() { // from class: com.xiaopeng.carcontrol.view.-$$Lambda$SpaceCapsuleActivity$eI55lk-dEsIn6nIeWyxYljZsBmg
                @Override // com.xiaopeng.xui.app.XDialogInterface.OnClickListener
                public final void onClick(XDialog xDialog2, int i) {
                    xDialog2.dismiss();
                }
            });
            this.mHandExitDialog.setCloseVisibility(true);
        }
        this.mHandExitDialog.show();
    }

    public /* synthetic */ void lambda$showHandSeatExitDialog$6$SpaceCapsuleActivity(XDialog xDialog, int i) {
        exitSpaceCapsule(false);
    }
